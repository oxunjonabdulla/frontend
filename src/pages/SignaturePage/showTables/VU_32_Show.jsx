...
import { IsImzo } from "../../../components";
import { useEffect, useState } from "react";
import UserApi from "../../../Service/module/userModule.api";
import { timeMoment } from "../../../utils/roleTest";

export const Vu_32Show = ({ vNumber }) => {

    const { data, setData } = useState([]);
    
    useEffect(() => {
        const fetchData = async () => {
            const { response } = await new UserApi().getVu32(vNumber);
        ...
        >
            ...
            <Tbody>
                <Tr>
                    ...
                    <Td rowSpan={8}>
                        <IsImzo isImzo={data?.data?.defestoskop_signature_user} />
                    </Td>
                    <Td rowSpan={8}>
                        <IsImzo isImzo={data?.data?.signature_image_url} />
                    </Td>
                    <Td rowSpan={8}>
                        <IsImzo isImzo={data?.data?.receiving_master_user_signature} />
                    </Td>
                    <Td rowSpan={8}>
                        <IsImzo isImzo={data?.data?.deputy_head_signature} />
                    </Td>
                </Tr>
