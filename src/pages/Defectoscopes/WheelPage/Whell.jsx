import { Box, Button, Flex, Heading, IconButton, Table, TableContainer, Tbody, Td, Text, Th, Thead, Tooltip, Tr, useDisclosure } from "@chakra-ui/react";
...
import { wheelDefc } from "../../../utils/mock_heads";
...
export const Wheel = () => {
    ...
    const handleDelate = async () => {
        const { response, error } = await new UserApi().deleteWheel(deletedID);
        if (response) window.location.reload();
        else toast({
            status: "error",
            title:
                error?.detail &&
                deletedID + " ma'lumot avval o'chirilgan",
            duration: 4000,
            isClosable: true,
            position: "top-right",
            fontSize: "3xl",
    ...
    const fetchData = async (page) => {
        setIsLoading(true);
        const { response } = await new UserApi().getWheelDefc(page);
        if (response) {
            setIsLoading(false);
        ...
        <Box
            ...
            {!isLoadingFulStatistik ? (
                !gettingData?.results?.length ? (
                    <Flex align={"center"} flexDir={"column"} my={20} gap={4}>
                    ...
                    <TableContainer p={4} border={"1px solid #eeeee"} pt={10}>
                        <Table borderRadius={10}
                            ...
                            <Thead bg={"#0c6170"} rounded={10}>
                                <Tr>
                                    {wheelDefc?.nestedHeaders.map((item, i) => (
                                        <Th
                                            fontSize={"12px"}
                                            textAlign={"center"}
                                            key={i}
                            ...
                            <Tbody>
                                {gettingData?.results?.map((item, i) => (
                                    <Tr key={i}>
                                        ...
                                        <Td fontSize={16} textAlign={"center"}>{item?.serial_number}</Td>
                                        <Td fontSize={16}>{timeMoment(item?.date)?.day}</Td>
                                        <Td fontSize={16} textAlign={"center"}>{item?.wheelset_axle_number}</Td>
            ...
            />
            <WheelModel isOpen={isOpen} onClose={onClose} />
        </Box>
    );
}