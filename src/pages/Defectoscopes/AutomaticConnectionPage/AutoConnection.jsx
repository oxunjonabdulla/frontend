...
import { AutoConnectModel } from "./AutoConnectModel";
import { autoConnect } from "../../../utils/mock_heads";
import { timeMoment } from "../../../utils/roleTest";

export const AutoConnection = () => {
    ...
    const handleDelate = async () => {
        const { response, error } = await new UserApi().deleteAutoConnect(deletedID);
    ...
    const fetchData = async (page) => {
        setIsLoading(true);
        const { response } = await new UserApi().getAutoConnect(page);
        if (response) {
            setIsLoading(false);
    ...
    return (
        <Box
                ...
                !gettingData?.results?.length ? (
                    <Flex align={"center"} flexDir={"column"} my={12} gap={4}>
                        ...
                        <Text
                            ...
                            fontSize={"2xl"}
                        >
                            Auto Birikma mavjud emas
                        </Text>
                        <Button colorScheme="teal" onClick={onOpen}>
                            Auto Birikma qo&apos;shish
                        </Button>
                    </Flex>
                ) : (
                    <TableContainer p={4} border={"1px solid #eeeee"}>
                        <Table borderRadius={10}
                            ...
                            <Thead bg={"#0c6170"} rounded={10}>
                                <Tr>
                                    {autoConnect?.nestedHeaders.map((item, i) => (
                                        <Th
                                            fontSize={"12px"}
                                            textAlign={"center"}
                                            key={i}
                            ...
                            <Tbody>
                                {gettingData?.results?.map((item, i) => (
                                    <Tr key={i}>
                                        ...
                                        <Td fontSize={16} textAlign={"center"}>{item?.repair_type}</Td>
                                        <Td fontSize={16} textAlign={"center"}>{item?.autocombination}</Td>
                                        <Td fontSize={16} textAlign={"center"}>{item?.traction_clamp}</Td>
                                        <Td fontSize={16} textAlign={"center"}>{item?.traction_clamp_clevis}</Td>
                                        <Td fontSize={16} textAlign={"center"}>{item?.pendulum_suspension}</Td>
                                        <Td fontSize={16} textAlign={"center"}>{item?.tension_bolt}</Td>
            ...
            />
            <AutoConnectModel isOpen={isOpen} onClose={onClose} />
        </Box>
    );
};
